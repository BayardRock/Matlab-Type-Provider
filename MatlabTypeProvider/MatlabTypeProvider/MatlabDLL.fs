...
type Engine = nativeptr<int>
type mxArray = nativeptr<int>
...
module LibEng =
    ...
    extern int engEvalString(Engine e, string cmd)

    [<DllImport("libeng.dll")>]
    extern void engSetVisible(Engine e, bool q)

    [<DllImport("libeng.dll")>]
    extern bool engGetVisible(Engine e)

    [<DllImport("libeng.dll")>]
    extern Engine engOpen(string startcmd)

    [<DllImport("libeng.dll")>]
    extern int* engClose(Engine e)    

    [<DllImport("libeng.dll")>]
    extern mxArray engGetVariable(Engine e, string name)

    [<DllImport("libeng.dll")>]
    extern int engPutVariable(Engine e, string name, mxArray array)

    [<DllImport("libeng.dll")>]
    extern int engOutputBuffer(Engine e, string output, int bufflen)


module LibMx =
    [<DllImport("libmx.dll")>]
    extern int* mxCreateDoubleMatrix(int n, int m, mxComplexity c)